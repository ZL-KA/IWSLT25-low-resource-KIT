...
class Trainer:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            ...
            if self.args.do_rdrop:
                assert self.args.loss_type=='x_loss' # Only support this one now
                original_losses, rdrop_logits = [], []
                for idx in range(self.args.num_pass):
                    # Only save gradients once
                    if idx == self.args.num_pass-1:
                        loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
                    else:
                        with torch.no_grad():
                            loss, outputs = self.compute_loss(model, copy.copy(inputs), return_outputs=True)
                    original_losses.append(loss)
                    rdrop_logits.append(outputs.logits)
                rdrop_pad_mask = (inputs['decoder_input_ids'] == 0)
                
                kl_div_loss = self.X_loss(rdrop_logits, rdrop_pad_mask)
                original_loss = sum(original_losses)/len(original_losses)
                loss = original_loss + self.args.kl_div_alpha*kl_div_loss
    ...
    def X_loss(self, logits, pad_mask):
        probs=[torch.nn.functional.softmax(raw_logit, dim=-1) for raw_logit in logits]
        pad_mask = pad_mask.view(-1)
        non_pad_mask = ~pad_mask
        dict_size = probs[0].size(-1)

        prob_avg=sum(probs)/len(probs)
        prob_avg=prob_avg.float().view(-1, dict_size)[non_pad_mask]

        kl_all = 0
        for the_prob in probs:
            the_prob = the_prob.float().view(-1, dict_size)[non_pad_mask]
            prob_diff = (the_prob-prob_avg) * (torch.log(the_prob) - torch.log(prob_avg))
            kl_all += prob_diff.sum()
        return kl_all/len(probs)
